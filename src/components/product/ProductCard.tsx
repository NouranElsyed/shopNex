...
const ProductCard = ({ product }: IProps) => {
  ...
  const { wishlistIds, toggleWishlist, loadingId, isLoading } = useWishlist();

  const isWishlisted = wishlistIds.has(id);
  const isWishlistLoading = isLoading && loadingId === id;
  ...
  const handleAddToCart = () => {
    ...
    if (!session?.user) {
      ...
      router.push(
        `/auth/login?callbackUrl=${encodeURIComponent(location.pathname)}`
      );
      return;
    }
      ...
      <article className="flex flex-col h-full">
        ...
        <div className="flex flex-col flex-1 p-4 space-y-3">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {isExpanded ? safeDescription : truncateText(safeDescription, 60)}
            ...
            {safeDescription.length > 60 && (
              <button
                ...
                className="ml-1 text-[#79ac31] font-medium text-xs"
              >
